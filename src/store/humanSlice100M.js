// src/store/humanSlice100M.js
import { createSlice } from "@reduxjs/toolkit";
import { ROOM_DIMENSIONS_100M } from "../constants/roomConstants100M";
...
const calculateInitialState = () => {
  ...
  Object.entries(ROOM_DIMENSIONS_100M).forEach(([roomName, config]) => {
...
const humanSlice100M = createSlice({
  name: "humans100M",
...
export const { addHuman, removeHuman, moveHuman, updateRoomCapacity } =
  humanSlice100M.actions;

// Selectors
export const selectRoomOccupancy100M = (state) =>
  state.humans100M.roomOccupancy;
export const selectPlacedHumans100M = (state) => state.humans100M.placedHumans;
export const selectRoomCapacity100M = (roomName) => (state) => {
  const room = state.humans100M.roomOccupancy[roomName];
  if (!room) return null;
...
export default humanSlice100M.reducer;
